...
export default {
  ...
  methods: {
    ...
    async becerileriYukle() {
      ...
      const cevap = await fetch(
        `https://vue-deneme-11-11-default-rtdb.europe-west1.firebasedatabase.app/beceriler.json`
      );

      const cevapVeriler = await cevap.json();

      if (!cevap.ok) {
        this.error = new Error("Kaynağa ulaşılamadı!");
      } else {
  ...
  computed: {
    ...
    //   return this.$store.getters["beceriler/sahipBeceriler"];
    // },
    beceriVarmi() {
      return this.beceriler.length > 0;
